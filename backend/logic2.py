...
def onLeaveRoom(sid):
    ...
    # If the player is the admin
    if ROOM.admin_id == sid:
        # makes all the players leave the room
        sio.emit("leave-room", room=ROOM_ID)
...
@sio.on("get-players")
def get_players(sid):
    sio.emit("get-players", data={"players": ROOM.get_players()}, to=sid)
